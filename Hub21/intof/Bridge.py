...
def cmdtask():
    ...
    while not TERMINATE:
        ...
        if (len(que) > 0):
            ...
            print ("dequeing: ", lgstatus)
            socketio.sleep (BUFFER_DELAY)  # to guarentee a minimum delay after LWT
            restore_device_status (json.loads(lgstatus))  #  if it was a dictionary, it will be passed by reference! 
    print ('\n * Commaner thread terminates. *\n')  
...
def discover_devices():
    ...
    if new_devices is None:
        return {'result' : False, 'error' : 'new device list is not available; please try again'}
    return new_devices         # NOTE: get/new/devices in Rouer has an entirely different logic
...
def auto_onboard_devices():
    ...
    if new_devices is None or len(new_devices)==0:
        return {'result' : False, 'error' : 'No new devices; please call discover/devices first'}
